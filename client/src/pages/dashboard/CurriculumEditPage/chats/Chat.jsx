  ...
  return (
    <div className="flex flex-col h-full w-full pt-4 relative">
      <div className="flex-grow overflow-auto p-4 gap-y-4">
        {messages.map((message) => (
          <div key={message.id} className="flex border rounded-lg p-2">
            <span className="text-sm-custom font-bold text-gray-700">
              {message.user}:
            </span>
      ...
      <form
        ...
        className="flex items-center mt-4 p-4 bg-primary-50 rounded-lg"
      >
        <input
          type="text"
          value={newMessage}
        ...
        <button
          type="submit"
          className="ml-2 px-4 py-2 text-white bg-blue-500 rounded-lg font-bold hover:bg-blue-600 active:bg-blue-700"
        >
          Send
        </button>
      </form>
